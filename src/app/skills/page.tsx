  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {skills.map((s) => (
          <details key={s.key} className="rounded-2xl border border-[var(--color-line)] bg-[var(--color-surface)] p-4">
            <summary className="cursor-pointer list-none">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <span className="font-medium">{s.name}</span>
                  <code className="ms-2 text-xs text-[var(--color-muted)]">{s.key}</code>
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  <Badge tone={s.autonomy === "auto" ? "ok" : "medium"}>{s.autonomy}</Badge>
                  <span className="text-xs text-[var(--color-muted)]">v{s.version}</span>
                </div>
              </div>
              <p className="mt-1 text-xs text-[var(--color-muted)]">{s.description}</p>
            </summary>
            <pre className="mt-3 overflow-x-auto border-t border-[var(--color-line)] pt-3 text-xs whitespace-pre-wrap">
              {s.body}
            </pre>
          </details>
        ))}
      ...
      <section>
        ...
        <div className="rounded-2xl border border-[var(--color-line)] bg-[var(--color-surface)] p-4">
          {automations.map((a) => (
            <div key={a.id} className="border-b border-[var(--color-line)] py-2.5 last:border-0">
              <div className="flex items-center justify-between gap-3">
