    ...
    <div className="space-y-4">
      ...
      <Card title="Store integration">
        {Object.values(CHAINS).map((c) => {
          ...
          return (
            <div key={c.key} className="border-b border-[--color-line] py-2.5 last:border-0">
              <div className="flex items-center justify-between gap-2">
                ...
                <Badge
                  tone={
                    c.cartAutomation === "supported"
                      ? "ok"
                      : c.cartAutomation === "experimental"
                        ? "medium"
                        : "low"
                  }
                >
                  basket: {c.cartAutomation}
                </Badge>
              </div>
              <p className="mt-1 text-xs text-[--color-muted]">{c.notes}</p>
