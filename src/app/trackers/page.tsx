        ...
        {trackers.map((t) => {
          ...
          return (
            <Link
              key={t.key}
              href={`/trackers/${t.key}`}
              className="rounded-2xl border border-[var(--color-line)] bg-[var(--color-surface)] p-4 transition hover:border-[var(--color-accent)]"
            >
              <div className="flex items-center gap-2">
                <span className="text-xl">{t.icon ?? "📋"}</span>
                <span className="font-medium">{t.name}</span>
              </div>
              <p className="mt-1 line-clamp-2 text-xs text-[var(--color-muted)]">{t.description}</p>
              <p className="mt-2 text-xs text-[var(--color-muted)]">
                <strong className="text-[var(--color-ink)]">{active}</strong> available · {total} total
              </p>
            </Link>
          );
        })}
