...
## ויקטורי — נבדק ונזנח 2026-09-10

נבדק לבקשת ישי ונעצר בהחלטתו ("לא עובד לי כרגע. עזוב"). נרשם כדי
שלא ייחקר מאפס שוב. כל שורה כאן נמדדה, לא הוסקה.

**החנות חסומה ב-Cloudflare Turnstile.** `www.victoryonline.co.il`
מחזיר HTTP 403 עם הכותרת "רק רגע..." וטוען
`challenges.cloudflare.com/turnstile`. אין טופס התחברות בדף — לא שדה
משתמש, לא סיסמה, לא SMS. **פרטי ההתחברות קיימים ב-`.env`
(`VICTORY_USERNAME`/`VICTORY_PASSWORD`) והם לא הבעיה**; פשוט אין לאן
להזין אותם. עקיפת אנטי-בוט היא מחוץ לתחום — המסלול הלגיטימי היחיד הוא
זה שעבד ב"בהצדעה": דפדפן אמיתי ב-noVNC, ישי עובר את האתגר ומתחבר
בעצמו, ואנחנו שומרים את הסשן.

**ויקטורי איננה בפורטל המשותף** `url.publishedprices.co.il` שממנו
נמשכות שבע רשתות. נוסו שישה איותים (victory, Victory, victoryonline,
VictoryOnline, vicory, Vicory) — כולם נדחו בהתחברות.

**היא כן מפרסמת ב-`laibcatalog.co.il`** (תוכנת "ניביט"). אומת בדפדפן
אמיתי: רשת `7290696200003` = ויקטורי, תת-רשת `7290696200003001`,
ו-**סניף `7290696200003001097` = "אינטרנט 97"** — סניף האונליין, שהוא
הרלוונטי למשק הבית (החיוב של ₪656.60 מ-2026-08-03 הוא "ויקטורי
אונליין", וזו גם הרשת שמופיעה ברשימת כרטיס ההטבות).

**ואף קובץ לא ירד.** ארבע דרכים, כולן ריקות:
1. POST ישיר עם `__VIEWSTATE`/`__EVENTVALIDATION` → HTTP 500.
2. ממשק ה-REST המתועד ב-`/Content/instructions.txt`
   (`?code=...&fileType=pricefull`) → טבלה עם שורת כותרת בלבד.
3. אותו ממשק עם `date=dd/MM/yyyy`, היום ואתמול → זהה.
4. דפדפן אמיתי שבוחר בתפריטים ולוחץ "חיפוש", כולל ברמת רשת שלמה עם
   "הכל" → שורת כותרת בלבד.

**ישי בדק אצלו בדפדפן וגם אצלו לא עבד.** זה מעביר את החשד מאיתנו
לאתר, אבל לא מוכיח: לא נבדק אם זו תקלה זמנית או שוויקטורי לא מוגשת
משם כרגע. **מה שלא נבדק:** פרסום חלופי (matrixcatalog לא נפתר מהשרת),
והרשימה הרשמית ב-gov.il של כתובות פרסום המחירים לפי רשת. זו הנקודה
להתחיל בה אם חוזרים לזה.
