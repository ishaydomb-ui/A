...
| מתאם שופרסל (Playwright) | ✅ **אומת מול החשבון האמיתי** — login, חיפוש, הוספה והסרה מהסל |
| התחברות אוטומטית + רענון session | ✅ headless עם שם משתמש+סיסמה, בלי OTP |
| תור למחזור כשהיציאה הישראלית כבויה | ✅ נשמר ומורץ אוטומטית כשהחיבור חוזר |
| מתאם טיב טעם | ⛔ לא מומש (החסימה נפתרה, אבל המתאם עצמו לא נכתב) |
...
בטלגרם ([@ClaudeGroceriesBot](https://t.me/ClaudeGroceriesBot)) — **מדברים
בשפה חופשית, בלי פקודות**:

| מה כותבים | מה קורה | עובד? |
|---|---|---|
| "תוסיף 300 גרם פסטרמה" | נוסף לרשימה עם משקל ויחידה | ✅ |
| "צריך טונה סטארקיסט 4 יחידות" | נשמר גם היצרן המועדף | ✅ |
| "תוריד את הטונה" | הסרה מהרשימה (התאמה מטושטשת) | ✅ |
| "מה יש ברשימה" | הרשימה המלאה: בסיס + תוספות | ✅ |
| "כמה עולה קוטג" | מחיר נוכחי בסניף + מבצע אמיתי | ✅ |
| "מה יש במבצע" | מבצעים על פריטי הרשימה | ✅ |
| "מתכון לפאי תפוחים" | מפרק למצרכים ומוסיף לרשימה | ✅ |
| "תכנן לי תפריט שבועי" | 5 ארוחות + רשימה מאוחדת | ✅ |
| "/start_order" | ממלא עגלה אמיתית בשופרסל | ⛔ חסום גיאוגרפית |

### איך ההבנה עובדת

`grocery_bot/nlu.py` מריץ `claude -p` — כלומר משתמש ב-Claude Code
**שכבר מותקן ומחובר על השרת**, בלי API key נפרד ובלי חיוב נוסף. כל
הודעה מסווגת ל-intent ומחולצים ממנה שם המוצר, כמות, יחידה ויצרן.

זמן תגובה ~7-10 שניות (הבוט מציג "מקליד..."). אם ה-CLI לא זמין או נתקע,
יש נפילה חזרה למנתח מבוסס-כללים — פחות חכם, אבל **לא** מוסיף זבל
לרשימה: הודעה לא ברורה מקבלת "לא הבנתי" במקום להפוך לפריט.

> הבאג שזה תיקן: הגרסה הראשונה שמרה כל הודעה כפריט מילולי, אז
> "תוסיף גבינה בולגרית" נשמר בתור פריט בשם *"תוסיף גבינה בולגרית"*,
> ו-"מה" נשמר כפריט בשם "מה".
